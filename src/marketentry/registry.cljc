(ns marketentry.registry
  ...
  Unlike BGR's sibling registry, this namespace carries NO numeric
  statutory-threshold recompute of its own -- AZE's flagship governor
  check (`marketentry.governor/unreliable-supplier-listed-violations`)
  is a plain registry-membership boolean read off the engagement record,
  not a computed formula, so there is nothing here for it to call. This
  is a deliberate, honestly-reported difference in check SHAPE, not an
  oversight -- see `marketentry.governor`'s namespace docstring.
