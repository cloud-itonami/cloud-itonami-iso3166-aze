(ns marketentry.marketentryllm
  ...
  the Azerbaijan (AZE) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "voen-verified?=" (:voen-verified? e)
                        " on-unreliable-supplier-registry?=" (:on-unreliable-supplier-registry? e)
