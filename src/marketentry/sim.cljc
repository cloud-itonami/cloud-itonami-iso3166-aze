...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (AZE, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up unreliable-supplier-listed) ==")
    ...
    (println "== filing/submit eng-4 (unreliable-supplier-listed -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up voen-unverified) ==")
    ...
    (println "== filing/submit eng-5 (voen-unverified -> HARD hold) ==")
