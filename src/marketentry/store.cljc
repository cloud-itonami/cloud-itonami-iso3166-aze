(ns marketentry.store
  "SSoT for the Azerbaijan (AZE) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:on-unreliable-supplier-registry?` / `:requires-voen?` /
  `:voen-verified?` are ground truth for the flagship and conditional
  governor checks."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Fabrication MMC" :portal "etender.gov.az"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-unreliable-supplier-registry? false
             :requires-voen? true :voen-verified? true
             :drafted? false :submitted? false
             :jurisdiction "AZE" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "etender.gov.az"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-unreliable-supplier-registry? false
             :requires-voen? true :voen-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "etender.gov.az"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "AZE" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export MMC" :portal "etender.gov.az"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-unreliable-supplier-registry? true
             :requires-voen? true :voen-verified? true
             :drafted? false :submitted? false
             :jurisdiction "AZE" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "etender.gov.az"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-unreliable-supplier-registry? false
             :requires-voen? true :voen-verified? false
             :drafted? false :submitted? false
             :jurisdiction "AZE" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "etender.gov.az"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
  ...
  (cond-> {:engagement/id id}
    ...
    (some? on-unreliable-supplier-registry?) (assoc :engagement/on-unreliable-supplier-registry? on-unreliable-supplier-registry?)
    (some? requires-voen?)                (assoc :engagement/requires-voen? requires-voen?)
    (some? voen-verified?)                (assoc :engagement/voen-verified? voen-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/on-unreliable-supplier-registry?
   :engagement/requires-voen? :engagement/voen-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :on-unreliable-supplier-registry? (boolean (:engagement/on-unreliable-supplier-registry? m))
     :requires-voen? (boolean (:engagement/requires-voen? m))
     :voen-verified? (boolean (:engagement/voen-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-aze"})
