  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Fabrication MMC"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Fabrication MMC" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest unreliable-supplier-listed-is-held-and-unoverridable
  (testing "unreliable-supplier-registry membership -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:unreliable-supplier-listed} (-> (store/ledger db) last :basis)))
...
(deftest voen-unverified-is-held-and-unoverridable
  (testing "unverified VÖEN (Taxpayer Identification Number) when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:voen-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
